...
Array.from(networkTypeToDirectoryMap.keys()).forEach((networkType) => {
  networkTypeToZoneFileMap.set(
    networkType,
    path.join(root, networkTypeToDirectoryMap.get(networkType), zoneFileName)
  );
});
...
function main() {
  ...
  Array.from(networkTypeToZoneFileMap.keys()).forEach((networkType) => {
    let zoneJson = chain_reg.readJsonFile(
      networkTypeToZoneFileMap.get(networkType)
    );
    zoneJson.assets.forEach((zoneAsset) => {
      let ASSET_EXISTS = false;
      chainRegAssetPointers.forEach((chainRegAsset) => {
        if (
          chainRegAsset.chain_name == zoneAsset.chain_name &&
          chainRegAsset.base_denom == zoneAsset.base_denom
        ) {
          ASSET_EXISTS = true;
        }
      });
      ...
      if (!ASSET_EXISTS) {
        throw new Error(
          `Asset ${zoneAsset.base_denom} does not exist in the chain registry.`
        );
      }
    });
  });
